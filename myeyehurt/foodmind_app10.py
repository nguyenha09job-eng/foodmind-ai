...
/* Vùng cuộn (chứa danh sách đồ ăn) */
.scroll-content {
  flex: 1;
  overflow-y: auto;
  /* Padding bottom 280px đảm bảo khi cuộn đến cuối, thẻ bị che sẽ trồi lên hẳn bảng tóm tắt */
...
/* =========================================
   BẢNG TÓM TẮT NHU CẦU (Nổi & Trong suốt)
   ========================================= */
.needs-card {
  position: absolute;
  bottom: 85px; /* Nằm ngay trên thanh điều hướng Bottom Nav */
  left: 16px;
  right: 16px;
  
  /* Hiệu ứng nền trong suốt mờ (Glassmorphism) */
  ...
  border-radius: 24px;
  padding: 18px 20px;
  box-shadow: 0 -4px 24px rgba(0,0,0,0.06), 0 10px 24px rgba(0,0,0,0.08);
  z-index: 100; /* Z-index cao để luôn đè lên phần scroll-content */
}
